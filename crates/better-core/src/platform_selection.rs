...
pub fn validate_install_target(os: &str, cpu: &str) -> Result<(), String> {
    if ![
        "darwin", "linux", "win32", "freebsd", "openbsd", "netbsd", "aix", "sunos", "android",
    ]
    .contains(&os)
    {
    ...
    if ![
        "arm", "arm64", "ia32", "x64", "loong64", "mips", "mipsel", "ppc", "ppc64", "riscv64",
        "s390", "s390x",
    ]
    .contains(&cpu)
    {
...
pub(crate) fn lookup_target(
    from: &str,
    name: &str,
    peer: bool,
    locations: &BTreeMap<&str, usize>,
) -> Option<usize> {
    ...
    loop {
        let candidate = if context.is_empty() {
            ...
            format!("{}/node_modules/{}", context, name)
        };
        if let Some(index) = locations.get(candidate.as_str()) {
            return Some(*index);
        }
        if context.is_empty() {
            return None;
...
fn edges_for(
    ...
    for name in metadata.dependencies.keys() {
        declarations.insert(name.as_str(), (false, false));
    }
    if root && !production {
        for name in metadata.dev_dependencies.keys() {
            declarations.entry(name.as_str()).or_insert((false, false));
        }
    }
    for name in metadata.optional_dependencies.keys() {
        declarations.insert(name.as_str(), (true, false));
    }
    ...
    for (name, (optional, peer)) in declarations {
        match lookup_target(from, name, peer, locations) {
            ...
            None => {
                return Err(format!(
                    "Lockfile entry '{}' is missing required dependency '{}'",
                    from, name
                ))
            }
        }
...
pub fn select_platform_packages(
    ...
    if let Some(root) = &resolved.root_selection {
        if !matches_platform(os, &root.os) || !matches_platform(cpu, &root.cpu) {
            return Err(format!(
                "Root package is incompatible with install target {}/{}",
                os, cpu
            ));
        }
        if root.libc.is_some() {
            return Err(
                "Native install does not support root libc restrictions; use npm install"
                    .to_string(),
            );
        }
    }
    ...
    for (index, package) in packages.iter().enumerate() {
        ...
        if !matches_platform(os, &metadata.os) || !matches_platform(cpu, &metadata.cpu) {
            if !metadata.optional {
                return Err(format!(
                    "Required package '{}' is incompatible with install target {}/{}",
                    package.rel_path, os, cpu
                ));
            }
    ...
    if excluded.is_empty() {
        return finish_selection(packages);
    }
    let root = resolved
        .root_selection
        .as_ref()
        .ok_or_else(|| "Platform selection requires lockfile root package metadata".to_string())?;
    let locations: BTreeMap<_, _> = packages
        .iter()
        .enumerate()
        .map(|(i, p)| (p.rel_path.as_str(), i))
        .collect();
    let graph: Vec<_> = packages
        .iter()
        .map(|p| edges_for(&p.rel_path, &p.selection, false, production, &locations))
        .collect::<Result<_, _>>()?;
    ...
    let mut required_parents = vec![Vec::new(); packages.len()];
    for (parent, edges) in graph.iter().enumerate() {
        for edge in edges.iter().filter(|edge| !edge.optional) {
            required_parents[edge.to].push(parent);
        }
    }
    let mut failures: VecDeque<_> = excluded.iter().copied().collect();
    while let Some(child) = failures.pop_front() {
        for &parent in &required_parents[child] {
            if excluded.contains(&parent) {
                continue;
            }
            if !packages[parent].selection.optional {
                return Err(format!(
                    "Required package '{}' depends on an excluded platform package",
                    packages[parent].rel_path
                ));
            }
            excluded.insert(parent);
            failures.push_back(parent);
        }
    }
    if root_edges
        .iter()
        .any(|edge| !edge.optional && excluded.contains(&edge.to))
    {
    ...
    while let Some(index) = queue.pop_front() {
        if excluded.contains(&index) || !reachable.insert(index) {
            continue;
        }
...
fn finish_selection(packages: Vec<ResolvedPackage>) -> Result<Vec<ResolvedPackage>, String> {
    for package in &packages {
        if package.selection.libc.is_some() {
            return Err(format!(
                "Native install does not support libc restrictions for '{}'; use npm install",
                package.rel_path
            ));
        }
    }
