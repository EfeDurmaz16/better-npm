...
/// Cache provenance schema after all supported digest algorithms became enforced.
pub const VERIFIED_MARKER: &str = "better-sri-v2";
...
impl Integrity {
    ...
    pub fn parse(value: &str) -> Result<Self, String> {
        ...
        if digest.len() != length {
            return Err(format!(
                "Invalid {} integrity: expected {} digest bytes",
                algorithm, length
            ));
        }
    ...
    pub fn verify_reader(&self, mut reader: impl std::io::Read) -> Result<(), String> {
        ...
        loop {
            let count = reader
                .read(&mut buffer)
                .map_err(|e| format!("Failed to read integrity input: {e}"))?;
            if count == 0 {
                break;
            }
...
mod tests {
    ...
    fn rejects_unsafe_or_unsupported_identities() {
        for input in [
            ...
            "sha999-AQID",
            "sha512-💣",
            "sha512-AAAA sha1-AAAA",
            "sha1-AAAAAAAAAAAAAAAAAAAAAAAAAAA=?option",
        ] {
