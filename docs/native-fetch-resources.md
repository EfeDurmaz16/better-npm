...
Extraction deliberately decompresses twice: a bounded preflight validates the archive before the extraction pass writes its entries. This trades decompression CPU for predictable validation before writes. Sparse archives, duplicate local PAX size records, and local PAX entries whose declared size differs from the underlying header are unsupported and fail explicitly before extraction writes. Ordinary GNU longnames and matching local PAX size metadata remain supported. Global PAX headers are accepted within the metadata and entry budgets, but their metadata is ignored, matching the extraction library; the preflight likewise applies size metadata only from local PAX headers.
